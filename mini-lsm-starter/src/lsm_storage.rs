...
impl LsmStorageInner {
        ...
        if !manifest_path.exists() {
            ...
            if options.enable_wal {
                for memtable_id in memtables {
                    ...
                    max_ts =
                        max_ts.max(memtable.map.iter().map(|x| x.key().ts()).max().unwrap_or(0));
    ...
    pub fn get_with_ts(&self, _key: &[u8], _read_ts: u64) -> Result<Option<Bytes>> {
        ...
        let iter = self.scan_with_ts(Bound::Included(_key), Bound::Included(_key), _read_ts)?;
    ...
    fn create_sstable_iterator_for_scan(
        ...
        let iter = match lower {
            ...
            Bound::Included(k) => SsTableIterator::create_and_seek_to_key(
                sst,
                KeySlice::from_slice(k, TS_RANGE_BEGIN),
            )?,
            Bound::Excluded(k) => {
                let mut it = SsTableIterator::create_and_seek_to_key(
                    sst,
                    KeySlice::from_slice(k, TS_RANGE_END),
                )?;
                while it.is_valid() && it.key().key_ref() == *k {
                    it.next()?;
                }
                it
            }
    ...
    fn build_l0_sstable_iters_for_scan(
        ...
        for id in snapshot.l0_sstables.iter() {
            ...
            if !Self::sstable_overlaps_bounds(
                sst.first_key().as_key_slice().key_ref(),
                sst.last_key().as_key_slice().key_ref(),
                lower,
                upper,
            ) {
                continue;
    ...
    fn build_leveled_sstable_iters_for_scan(
        ...
        for (_level, files) in &snapshot.levels {
            ...
            let iter = match lower {
                ...
                Bound::Included(k) => SstConcatIterator::create_and_seek_to_key(
                    sstables,
                    KeySlice::from_slice(k, TS_RANGE_BEGIN),
                )?,
                Bound::Excluded(k) => {
